...
func (db *Db) UserContextByTgId(botId int64, userId int64) (*bctx.Context, error) {
	...
	query := `SELECT context FROM ` + prefix + `.user WHERE tg_id = $1;`
	...
	return bctx.NewContextFromJSON(*ctx)
}

func (db *Db) SetUserContextByTgId(botId int64, userId int64, ctx *bctx.Context) error {
	prefix := prefixSchema + strconv.FormatInt(botId, 10)
	bytes, err := ctx.ToJSON()
	if err != nil {
		return nil
	}
	query := `UPDATE ` + prefix + `.user SET context = $1 WHERE tg_id = $2;`
	_, err = db.Pool.Exec(context.Background(), query, bytes, userId)
	return err
}
